...
public class ThreadMain { 											// inizio della classe Main
	...
	/* 
	 * 	Questo è il metodo dell'inzio 
	 *  Viene chiamato dal main  
	 *  Qui ci sono tutte le funzioni chiamate 
	 * 
	 */
	public static void inizio () {
		...
		for (int i = 0; i < nrClienti; i++) {										//Creazione di tutti i clienti con un nome , un fido e il banchiere in comune 
			
			C[i] = new Cliente(fidoClienti[i], B , "Cliente "+ (i+1) );
		} 
		
		for (int i = 0; i < nrClienti; i++) 										//Partenza di tutti i clienti con il metodo start 
		...
		for (int i = 0; i < nrClienti; i++) {
			...
			try {
				...
				C[i].join();														//Aspettare che tutti finiscano 
	...
	public static void controlloFidi() { 													// metodo della classe che controlla la somma dei fidi dei clienti coinvolti
		...
		if (sommaFidi == cassa) { 															// se la somma totale dei fidi è uguale al valore della cassa, allora fa'...
			...
			System.out.println(
					"In questo caso non c'è bisogno di fare l'algoritmo di Dijkastra, "
					+ "pigia 'y' per continuare..."); 										 //Stampa a video queste stringhe 
